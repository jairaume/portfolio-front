<template>
  <div v-if="left" class="experience grid px-4 h-fit relative">
    ...
    <h3 class="text-monument-h3 text-orange-100 text-right font-light s:absolute top-0 s:left-full s:w-full s:text-left s:px-8 s:translate-y-1/2">
      {{new Date(experience.start_date).getFullYear()}} – {{ (experience.end_date ? experience.end_date : $t('common.now')) }}
    </h3>
  ...
  <div v-else class="experience grid px-4 h-fit row-start-2 col-start-2 relative">
    ...
    <h3 class="text-monument-h3 text-orange-100 text-left font-light s:absolute top-0 s:right-full s:w-full s:text-right s:px-8 s:translate-y-1/2">
      {{new Date(experience.start_date).getFullYear()}} – {{ (experience.end_date ? experience.end_date : $t('common.now')) }}
    </h3>
    <h4 class="text-h4">{{experience["position_"+locale]}} · {{experience.company}}</h4>
    <p class="text-grey-50 text-sm font-light">
