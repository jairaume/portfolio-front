<template>
  <header ref="rootEl" id="menu" class="w-full z-50 sticky top-0 text-white">
    <nav class="relative responsive-padding-x py-8 flex items-center justify-between m:justify-center m:gap-6 drop-shadow">
      <nuxt-link :to="localePath('/')" class="text-cta px-8 hidden m:block">{{ $t('common.home') }}</nuxt-link>
      <nuxt-link :to="localePath('/about')" class="text-cta px-8 hidden m:block">{{ $t('common.about') }}</nuxt-link>
      <nuxt-link
          :to="localePath('/')"
      ...
      <nuxt-link :to="localePath('/projects')" class="text-cta px-8 hidden m:block">{{ $t('common.projects') }}</nuxt-link>
      <nuxt-link :to="localePath('/contact')" class="text-cta btn btn-black px-4 py-1 hidden m:block">
        {{ $t('common.contact') }}
      </nuxt-link>

      <div class="absolute right-0 mx-8">
        <button class="text-cta px-3 py-1 border border-white rounded-lg duration-300 m:hidden" :class="{'bg-white text-grey-500': expanded}" aria-controls="overlay" @click="toggleMenu">menu</button>
